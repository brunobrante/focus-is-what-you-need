...
- **Save-path behavior changes — SAVE-8, SAVE-10, SAVE-12, SHELL-4.** Each alters retry/backoff,
  queue coalescing, the `peekTable` contract, or merges the two scene-persistence hooks. All are
  data-loss-adjacent — better as a focused, test-backed effort. (**SAVE-11** and **SAVE-4** are now
  done — see below; `bun test` *is* available after all.)
