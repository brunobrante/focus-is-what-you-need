...
| STAGE-4 | ✅ fixed | Escape aborts an in-flight drag/resize/rotate/radius gesture: `useCanvasPointerEvents` exposes `cancelActiveInteraction` (reverts to `beforeDocument`, clears `commandModeRef`/drop-target/classes), called from the keyboard cancel branch via a ref. `UX.md` updated. |
...
| BLD-11 | ✅ | `RootSwitcher` thumbnail cache is now **refcounted**: `acquireThumbnail`/`releaseThumbnail` track mounted users per id and revoke + evict the `blob:` URL only when the last one unmounts — so the cache no longer leaks for the session, and a shared URL is never revoked while a sibling still shows it. Inline `reference.url` data URLs stay untracked (not ours to revoke). No new tsc errors. |
