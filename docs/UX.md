...
- **Path edit mode:** double-click a selected path (or press `Enter`) to edit it. The overlay draws the path's **skeleton** — selection-blue lines tracing every segment between anchors (sampled through curves) — with anchors as small squares and handles as round knobs on top. Drag an anchor or handle to move it; **double-click a segment** inserts an anchor; **Alt-click an anchor** removes it; double-click an anchor toggles corner ↔ smooth. `Esc`/`Enter` or clicking empty canvas leaves edit mode. The element's box/resize handles are hidden while editing.
