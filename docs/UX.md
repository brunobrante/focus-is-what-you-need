...
**Screen pages (fixed window, scrollable content — see the Product.md law):**
- The frame never grows; instead its **content** can span multiple **pages** along one axis. The frame stays at its device size and clips (`overflow: hidden`); the content slides behind it. Available on screens **and** components; not on icon masters (fixed 24×24 artboards) or the freeform sketch canvas.
- **Add page** (`+` pill, bottom-left beside the zoom control): on a single-page frame the click opens a small **drop with "Vertical" and "Horizontal"** — picking one creates page 2 and locks the content axis. Once pages exist, `+` adds another page on that axis directly (no drop). Deleting back to one page frees the axis; the next `+` asks again.
- **Pages rail** (miniature preview; appears only with 2+ pages): pinned to the **right edge** for vertical pages, or as a **filmstrip centered at the top** for horizontal pages. It shows a **live miniature of the whole content** — the real scene renderer scaled down (not a bitmap), refreshed ~200ms after edits settle so it never competes with a gesture for frames. Each page shows its number; a **blue rectangle** marks the window slice currently visible in the frame. Click/drag on the rail scrubs the scroll; the mouse wheel over it scrolls too.
- **Delete page**: hovering a page in the rail (except page 1) reveals an **×**. Page 1 is the screen itself and can never be deleted. Deleting an **empty** page is immediate; a page **with elements** opens a confirmation modal ("Delete page N?" — the elements on it are deleted and later pages shift back). The count decrement, deletions and shifts land in **one undo step**.
- **Scrolling the content**: hold the page-scroll modifier (**Alt/Option** by default; rebindable as `canvas.viewport.wheelPageScroll`, listed as "Scroll screen pages while wheeling") and wheel over the canvas — the content scrolls inside the fixed window instead of panning the view. On a horizontal axis a trackpad's sideways swipe drives it when dominant; a plain mouse wheel still works. Any content scroll (rail, scrub, or modifier+wheel) **dismisses the context toolbar** so it never floats over moving content.
- **Snap-to-page**: scrolling is free during the gesture, but once a rail scrub is released or a modifier+wheel settles (~140ms quiet), the scroll **eases to the nearest page boundary** (~220ms ease-out) so the window never rests half a page. A fresh scrub/wheel cancels an in-flight glide.
- **Editing across pages**: every tool works on any page — select, draw, text, pen/pencil, image drop, arrow-key nudge and Inspector X/Y all clamp to the **content** rect (device size × pages), not the window. Clicks land only through the **visible window slice**; the clipped area outside it reads as background.
- **Auto-scroll at the window edge**: while a drag, draw, or resize holds the pointer near the window's **leading/trailing edge along the content axis**, the content scrolls in that direction at a ramped rate (gentle just inside the edge zone, faster at the very edge), carrying the moved element into the neighbouring page hands-free. It stops the moment the pointer leaves the edge zone or the content reaches its start/end, and never triggers off the content axis.
- **Chrome clips to the window**: with 2+ pages the selection/hover outlines, resize & radius handles, guides and size tags are **clipped to the on-screen window** (padded by the handle reach), so an element scrolled out of the visible slice never leaves its handles floating over the stage background. The context toolbar is a DOM overlay and is unaffected.
- **Window-aware paste & align**: pasting SVG/clipboard markup centers on the **visible window slice** (the page you're looking at), not always page 1. Aligning a single **root** element to the frame (Inspector align row / align shortcut) aligns to the visible window too. Multi-selection align still uses the selection's own union; internal element paste (Cmd+V) still preserves the copied position (cascaded), only re-clamped inside the content.
- **Persistence**: the page count and axis are saved with the scene (on the frame's subject node); the **scroll position is transient** and resets when the subject reopens. Undo/redo restores the page count together with the elements. A paged component placed in a parent (or as a linked instance) renders its **window only, clipped, at scroll 0**.
...
- **Lasso** (`Q`) / **Paint**: select **multiple anchors** at once. Lasso drags a freeform region (drawn as a soft blue polygon) and selects every anchor inside it on release; Paint brushes selection onto anchors the cursor sweeps over. Hold **Shift** to add to the current selection. Selected anchors render filled. With **Move**, dragging any selected anchor moves the whole selection together; grabbing an unselected anchor collapses the selection to it. A bare click (or `Esc`) clears the selection.
- **Shape builder** (`M`): drag a stroke across the edited path's overlapping **subpaths** to combine the ones it touches — **union** by default, **Alt-drag** to subtract them from the first. (Combining separate *elements* is still the Inspector's multi-select boolean; this operates on subpaths of one path. Boolean geometry flattens curves to polygons, same as the Inspector ops. Needs the path to have ≥2 subpaths.)
- **Variable width** (`⇧W`): drag away from an anchor to taper the stroke there — the drag distance sets that anchor's width multiplier. Because SVG has no tapered stroke, a path with any variable width renders its stroke as a **filled outline** (a ribbon computed from the per-anchor widths) instead of a uniform stroke; open subpaths taper, closed subpaths keep the uniform stroke. Widths persist with the path. `Esc` cancels an in-flight width drag.
